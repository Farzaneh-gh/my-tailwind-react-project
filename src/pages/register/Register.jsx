      ...
      try{
       ...
       if(response.status === 201) {
        ...
        if(response.status===409) {
         swal({
            title: "خطا",
            text: "ایمیل یا شماره موبایل قبلا ثبت شده است",
            icon: "error",
            button: "باشه",
          });
            return;
        ...
        swal({ title: "خطا", text: "ثبت نام ناموفق بود", icon: "error", button: "باشه" });
      }
  };
    ...
    <main className="flex-center flex-col relative px-4 py-6 min-h-screen bg-gray-100 font-Dana">
      ...
      <div className="max-w-95 w-full pt-5 pb-6 px-6 text-center bg-white dark:bg-zinc-800 rounded-2xl shadow-lg">
        <div className="flex flex-col">
          <h4 className="font-MorabbaBold text-xl mb-4 sm:mb-4.5 text-zinc-700 dark:text-white">
            عضویت
          </h4>
          ...
          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            <div className="relative">
              <input
                ...
                {...register("name", { required: "نام کاربری الزامی است" ,maxLength: { value: 20, message: "نام کاربری نباید بیشتر از 20 کاراکتر باشد" }})}
             
              />
              <svg className="absolute left-3.5 top-3 w-5 h-5 text-gray-300">
                <use href="#icon-user" />
              </svg>
              {errors.username && (
                <span className="text-red-500 text-sm mt-2  block text-right mr-3">
